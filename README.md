...
1.1 Что такое Hadoop? - это платформа с открытым исходным кодом для надежной, масштабируемой, распределенной обработки больших наборов данных с помощью простых моделей программирования.
1.2 Что такое HDFS? - файловая система, предназначенная для хранения файлов больших размеров, поблочно распределённых между узлами вычислительного кластера.
1.3 * Что такое YARN? - это система планирования заданий и управления кластером
_
2.1 Какие минусы или опасные места HDFS? - 
    - Режим чтения и записи: файл может иметь только одну запись, поддерживает только добавление и не может быть изменен случайным образом.
    не подходит для доступа с малой задержкой: несколько раундов вызовов RPC
    - Маленький файл:
    - Информация метаданных хранится в памяти NameNode.
    - Метаданные блока занимают около 150 байт памяти, а для хранения 100 миллионов блоков требуется около 20 ГБ.
    - Если размер файла составляет 10 КБ, размер 100 миллионов файлов составляет всего 1 ТБ.
    - Память узла NameNode ограничена
    - Стремитесь получить большое количество небольших файлов и потребляйте много времени на поиск.
    - Копирование большого количества маленьких файлов вместо копирования большого файла
2.2 Что такое блок HDFS? - Размер блока HDFS по умолчанию составляет 128 МБ. Мы можем увеличить или уменьшить размер блока в соответствии с нашими потребностями. Это не похоже на файловую систему ОС, где размер блока составляет 4 КБ. Если размер данных меньше размера блока HDFS, то размер блока будет равен размеру данных.
2.3 Для чего используется NameNode? - Хранит в себе все метаданные системы — непосредственно маппинг между файлами и блоками
2.4 Для чего используется DataNode? - Для хранения блоков файлов
2.5 Что будет, если записать много маленьких файлов в HDFS? - Каждый файл займет один блок и место на dataNode закончиться быстрее
2.6 Что будет, если несколько DataNode внезапно отключатся? - Если у этих нод есть репликации, то задействуются они, если нет, то будет печалька
2.7 Как проадпейдить несколько записей в большом файле на hdfs? - можно записывать в начало или конец файла
2.8 * Почему задачи на YARN нестабильны? - 
_
3.1 Что такое Hive? - представляет из себя движок, который превращает SQL-запросы в цепочки map-reduce задач
3.2 Что хранит HiveMetastore? - Хранит метаданные
3.3 Чем отличается external table и managed table? - для external можно указать где хранится таблица, и при удалении удаляться только метаданные
3.4 * Какие форматы умеет читать Hive? - json, yaml, и тд
3.5 * Чем отличается управление ресурсов в Hive и Impala?
_
4.1 Чем отличается колоночный формат хранения данных от строчного? - колончатый занимает меньше места, процессы агригации происходят быстрее, но чтение и запись медленнее
4.2 Чем отличается parquet/orc от csv? - csv строчный, а parquet колончатый формат
4.3 Чем отличается Avro от json? - структурой
4.4 * Чем отличается документо-ориетированный формат данных от реляционного?
_
5.1 Чем отличается etl и elt? - порядком обработки и сохранения данных
5.2 Какие основные проблемы etl/elt архитектуры? - шаг обработки данных
5.3 * Какие инструменты etl/elt вы знаете? - spark, airflow
_
6.1 Для чего нужны key-value СУБД? - для хранения данных
6.2 * Какие сложности стриминга в hdfs?
6.3 * Какие минусы key-value хранилищ?
_
7.1 Из чего состоит хранилище данных? - Хранилище состоит из запоминающих устройств и их носителей, не доступных непосредственно процессору
7.2 Какие виды хранилищ данных вы знаете? - файловое хранилище, объектное хранилище и блочное хранилище
7.3 * Основные задачи Data governance?
